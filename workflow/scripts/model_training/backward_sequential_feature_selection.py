...
def SBFS(df_dataset, feature_table, model_name, epsilon, params, polynomial=False):
    ...
    for i in range(n_features-1): # iterate (# of feature) times
        ...
        best_aupr = 0
        best_precision = 0
        for k in range(len(feature_list)): # iterate through each feature, remove one with least effect
...
def SBFS_significance(df_dataset, feature_table, model_name, epsilon, params, polynomial=False, n_boot = 1000):
    ...
    df = pd.DataFrame(columns = ['feature_removed', 'aupr', 'delta_aupr', 'delta_aupr_low', 'delta_aupr_high', 'pval_aupr',
    ...
    res_precision = scipy.stats.bootstrap(data, lambda Y_true, Y_new: statistic_precision_at_threshold(Y_true, Y_new, thresh), n_resamples=n_boot, paired=True, confidence_level=0.95, method='BCa')
    aupr = np.mean(res_aupr.bootstrap_distribution)
    precision = np.mean(res_precision.bootstrap_distribution) 

    df_temp = pd.DataFrame({'feature_removed': 'None', 'aupr': aupr, 'delta_aupr': 0, 'delta_aupr_low': 0, 'delta_aupr_high': 0, 'pval_aupr': 1,
    ...
    for i in range(len(feature_list)):
        ...
        df_temp = pd.DataFrame({'feature_removed': to_remove, 'aupr': aupr, 'delta_aupr': delta_aupr, 'delta_aupr_low': res_delta_aupr.confidence_interval[0], 'delta_aupr_high': res_delta_aupr.confidence_interval[1], 'pval_aupr': pval_aupr,
