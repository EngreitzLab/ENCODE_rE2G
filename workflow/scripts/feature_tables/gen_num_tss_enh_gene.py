...
def  determine_num_tss_enh_gene(pred_df, ref_gene_tss, extended_enhancers, enhancer_tss_int, out_file):
	...
	pred_df.loc[downstream_enh.index, "new_end"] = pred_df.loc[downstream_enh.index, "end"]
	pred_df.loc[downstream_enh.index, "start"] = pred_df.loc[downstream_enh.index, "TargetGeneTSS"]

	#  File to intersect with TSS annotations to count #  protein-coding TSSs between enhancer and promoter  (0 = closest TSS)
	pred_df[["chr", "start", "new_end", "name", "TargetGene"]].to_csv(extended_enhancers, sep="\t", index=False)
...
def main(abc_predictions, ref_gene_tss, extended_enhancers, enhancer_tss_int, out_file):
	...
	pred_df = pred_df[pred_df["class"] != "promoter"] # filter out promoters
	if len(pred_df) == 0:
		raise Exception("Did not find any enhancers in the Predictions file")
	
	determine_num_tss_enh_gene(pred_df, ref_gene_tss, extended_enhancers, enhancer_tss_int, out_file)
